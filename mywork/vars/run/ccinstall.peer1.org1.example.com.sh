...
cd /go/src/github.com/chaincode/worker

  go env -w GOPROXY=https://goproxy.cn,direct

if [ ! -f "worker_go_1.0.tar.gz" ]; then
  cd go
  GO111MODULE=on
  go mod vendor
  cd -
  peer lifecycle chaincode package worker_go_1.0.tar.gz \
    -p /go/src/github.com/chaincode/worker/go/ \
    --lang golang --label worker_1.0
fi

peer lifecycle chaincode install worker_go_1.0.tar.gz
